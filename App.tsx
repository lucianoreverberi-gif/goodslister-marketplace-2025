...
import {
    ...
    AnnexFishingGearPage,
    AnnexDivingSnorkelingPage,
...
const App: React.FC = () => {
    ...
    // ... rest of state ...
    const [listingToEdit, setListingToEdit] = useState<Listing | undefined>(undefined); useEffect(() => { const p = new URLSearchParams(window.location.search); const bl = p.get("boost_success"); const bt = p.get("tier"); if (bl) { const tn: any = {local: "Local Boost (3 days)", spotlight: "Spotlight (7 days)", regional: "Regional Hero (14 days)"}; const tname = tn[bt || ""] || "Boost"; const NL = String.fromCharCode(10); setTimeout(function() { const ov = document.createElement("div"); ov.style.cssText = "position:fixed;inset:0;background:rgba(15,23,42,0.8);backdrop-filter:blur(4px);z-index:99999;display:flex;align-items:center;justify-content:center;padding:16px;"; ov.innerHTML = '<div style="background:white;border-radius:24px;box-shadow:0 25px 50px -12px rgba(0,0,0,0.5);max-width:480px;width:100%;padding:40px;text-align:center;"><div style="width:80px;height:80px;margin:0 auto 24px;background:#d1fae5;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:40px;">🚀</div><h2 style="font-size:28px;font-weight:900;color:#0f172a;margin:0 0 12px;">Boost Activated!</h2><p style="color:#475569;font-size:16px;margin:0 0 8px;"><strong>'+tname+'</strong> is now live for your listing.</p><p style="color:#64748b;font-size:14px;margin:0 0 32px;">Your listing will appear at the top of relevant searches. Check <strong>My Boosts</strong> tab for performance stats.</p><button id="boost-success-close" style="width:100%;padding:16px;background:#0891b2;color:white;border:none;border-radius:16px;font-size:16px;font-weight:700;cursor:pointer;">Awesome, lets go!</button></div>'; document.body.appendChild(ov); document.getElementById("boost-success-close").onclick = function() { ov.remove(); }; }, 800); window.history.replaceState({}, "", "/#userDashboard"); } }, []);
    ...
    const renderPage = () => { if ((page as any) === 'damageDetail') { return session?.id && selectedDamageReportId ? (<DamageDetailPage reportId={selectedDamageReportId} currentUserId={session.id} onBack={() => { window.location.hash = ''; setPage('userDashboard'); }} />) : <p className="p-8 text-center text-gray-500">Please log in to view damage reports.</p>; }
        switch (page) {
            ...
            case 'annexFishing':
                return <AnnexFishingGearPage />;
            case 'annexDiving':
                return <AnnexDivingSnorkelingPage />;
            case 'howItWorks':
