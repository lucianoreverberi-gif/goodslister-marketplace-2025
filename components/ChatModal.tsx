...
interface ChatInboxModalProps {
    ...
    currentUser: User;
    onSendMessage: (conversationId: string, text: string, targetLang: string) => void;
...
const ChatInboxModal: React.FC<ChatInboxModalProps> = ({ isOpen, onClose, conversations, currentUser, onSendMessage, initialConversationId }) => {
    ...
    const handleSend = () => {
        if (messageInput.trim() && activeConversationId) {
            onSendMessage(activeConversationId, messageInput, targetLang);
            setMessageInput('');
        }
    };
                        ...
                        <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-gray-100">
                            {activeConversation.messages.map(msg => (
                                <div key={msg.id} className={`flex flex-col ${msg.senderId === currentUser.id ? 'items-end' : 'items-start'}`}>
                                    ...
                                    {msg.originalText && (
                                        <p className="text-xs text-gray-400 mt-1 italic">Original: "{msg.originalText}"</p>
                                    )}
                                </div>
                            ))}
