...
const HomePage: React.FC<HomePageProps> = ({ onListingClick, onCreateListing, listings, heroSlides, banners, categoryImages }) => {
    ...
    useEffect(() => {
        ...
        if (activeFilters.category) {
            results = results.filter(l => l.category === activeFilters.category);
        }
        ...
        if (activeFilters.text) {
            const textLower = activeFilters.text.toLowerCase();
            results = results.filter(l => 
                l.title.toLowerCase().includes(textLower) ||
                l.description.toLowerCase().includes(textLower)
            );
        }
    ...
    const handleCategoryClick = (category: string) => {
        setActiveFilters({ category: category as ListingCategory });
        setSearchQuery('');
    };
    ...
    const showSearchResults = Object.keys(activeFilters).length > 0;
                ...
                <div className="bg-gray-50">
                    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={clearFilters} className="text-sm font-medium text-cyan-600 hover:text-cyan-800">Clear filters and go back</button>
                        </div>
