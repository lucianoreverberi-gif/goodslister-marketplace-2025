...
const HomePage: React.FC<HomePageProps> = ({ 
    ...
    const performSearch = async (query: string) => {
        ...
        try {
            // Use Neural Search v2 for full location/category/date/price extraction
            const criteria = await processSearchQuery(query);
            onSearch(criteria);
        } catch (err) {
            console.error('Neural search failed, falling back to text search:', err);
            // Fallback: send raw query as text filter
            onSearch({ text: query } as any);
        }
        setIsSearching(false);
    };
    ...
    return (
        <main>
            ...
            <>
                <div className="flex flex-col">{/* How it Works Section */}
