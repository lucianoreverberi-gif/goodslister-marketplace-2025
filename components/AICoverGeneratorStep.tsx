...
import { WandSparklesIcon, CheckCircleIcon, AlertIcon, LockIcon, PaletteIcon, TagIcon, LightbulbIcon } from './icons';
...
const INSPIRATION_CHIPS = {
    vibes: [
        { label: '🏝️ Tropical', text: 'situated in a tropical paradise with palm trees and turquoise water' },
        { label: '⛰️ Mountain', text: 'parked on a rugged mountain trail with snow-capped peaks in the background' },
        { label: '🏙️ Urban', text: 'in a high-end marina with city skyline lights reflecting on the water' },
        { label: '🌲 Forest', text: 'surrounded by ancient pine trees and ferns in a misty forest' },
    ],
    lighting: [
        { label: '🌅 Golden Hour', text: 'bathed in warm, golden hour sunset lighting' },
        { label: '🌙 Moonlight', text: 'under a dramatic moonlit sky with cinematic shadows' },
        { label: '☀️ Midday', text: 'under bright, clear blue skies with sharp details' },
    ],
    action: [
        { label: '🌊 Splashing', text: 'splashing through waves with dynamic water spray' },
        { label: '💨 Fast', text: 'captured in motion with motion blur background' },
        { label: '📸 Studio', text: 'isolated on a clean, professional studio infinity background' },
    ]
};
...
const AICoverGeneratorStep: React.FC<AICoverGeneratorStepProps> = ({ category, realPhotoCount, onImageGenerated }) => {
    ...
    // Auto-fill product type based on category
    useEffect(() => {
        if (category && !productType) {
            setProductType(category.toLowerCase().replace('_', ' '));
        }
    }, [category]);
                ...
                <div className="relative p-8 flex flex-col items-center text-center z-10">
                    ...
                    <div className="mt-4 p-4 bg-white/80 backdrop-blur-md rounded-lg border border-amber-200 max-w-lg shadow-sm">
                        <div className="flex items-start gap-3">
                            ...
                            <p className="text-amber-900 text-sm font-medium text-left">
                                To maintain trust, you must upload at least <strong>3 real photos</strong> of your item's actual condition before you can generate an AI cover image.
                            </p>
                        </div>
                    </div>
                    <div className="mt-6 flex items-center gap-3">
                        ...
                        <div className="flex gap-1">
                            {[1, 2, 3].map((step) => (
                                <div key={step} className={`h-2 w-8 rounded-full ${realPhotoCount >= step ? 'bg-green-500' : 'bg-gray-300'}`} />
                            ))}
                        </div>
                        <span className="text-xs font-bold text-gray-600 ml-2">{realPhotoCount}/3 Photos</span>
                    </div>
                </div>
            </div>
        );
        ...
        <div className="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 rounded-xl p-1 border border-purple-100 shadow-md my-8">
            <div className="bg-white/90 backdrop-blur-sm rounded-lg p-6">
                ...
                <div className="flex items-center gap-3 mb-8 border-b border-indigo-100 pb-4">
                    ...
                    <span className="ml-auto bg-indigo-100 text-indigo-700 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wide border border-indigo-200">Unlocked</span>
                </div>
                ...
                <div className="mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="text" value={productType} onChange={e => setProductType(e.target.value)} placeholder="e.g. Jet Ski" className="w-full text-sm border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500" />
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-gray-500 mb-1">Brand & Model <span className="text-red-500">*</span></label>
                            <input type="text" value={brandModel} onChange={e => setBrandModel(e.target.value)} placeholder="e.g. Yamaha VX" className="w-full text-sm border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500" />
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-gray-500 mb-1">Color & Details <span className="text-red-500">*</span></label>
                            <input type="text" value={colorFeatures} onChange={e => setColorFeatures(e.target.value)} placeholder="e.g. Blue with silver trim" className="w-full text-sm border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500" />
                        </div>
                    </div>
                </div>
                ...
                <div className="mb-6">
                    ...
                    <div className="mt-4 space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {[...INSPIRATION_CHIPS.vibes, ...INSPIRATION_CHIPS.lighting, ...INSPIRATION_CHIPS.action].map((chip, idx) => (
                ...
                {!generatedImage && (
                    <button 
                        ...
                        className="w-full py-4 bg-gray-900 hover:bg-black text-white font-bold rounded-xl shadow-lg transition-all transform hover:scale-[1.01] active:scale-[0.99] flex justify-center items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {isGenerating ? (
                            <>
                            ...
                            <>
                                ...
                                {isFormValid ? "Generate Cover Image" : "Fill Details to Generate"}
                            </>
                        )}
                    </button>
                )}
                ...
                {error && (
                    <div className="mt-4 p-3 bg-red-50 text-red-700 text-sm rounded-lg border border-red-100 text-center flex items-center justify-center gap-2">
                    ...
                    <div className="mt-8 bg-white p-1 rounded-xl border border-gray-200 shadow-lg animate-in fade-in zoom-in-95 duration-300">
                        <div className="relative aspect-video w-full overflow-hidden rounded-lg bg-gray-100 group">
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
                                <span className="text-white text-xs font-medium bg-black/50 px-2 py-1 rounded backdrop-blur-sm">AI Preview</span>
                            </div>
                        </div>
                        ...
                        <div className="p-4">
                            <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-100">
                                <label className="flex items-start gap-3 cursor-pointer">
                                    <input type="checkbox" checked={hasAgreed} onChange={(e) => setHasAgreed(e.target.checked)} className="mt-0.5 h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
                                    <span className="text-xs text-blue-900 leading-snug">
                                        I confirm this image reasonably represents the <strong>{colorFeatures} {brandModel}</strong> I am listing.
                                    </span>
                                </label>
                            </div>

                            <div className="flex gap-3">
                                <button onClick={() => setGeneratedImage(null)} className="flex-1 py-2.5 border border-gray-300 text-gray-600 text-sm font-semibold rounded-lg hover:bg-gray-50 transition-colors">Discard</button>
                                <button onClick={handleApply} disabled={!hasAgreed} className="flex-1 py-2.5 bg-green-600 text-white text-sm font-bold rounded-lg hover:bg-green-700 disabled:opacity-50 shadow-sm flex justify-center items-center gap-2 transition-all">
                                    <CheckCircleIcon className="h-4 w-4" /> Use as Cover
                                </button>
                            </div>
                        </div>
                    </div>
