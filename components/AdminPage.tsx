...
const ExportCsvSection: React.FC = () => {    const [downloading, setDownloading] = React.useState<string | null>(null);    const email = 'lucianoreverberi@gmail.com';    const types = [        { id: 'bookings', label: 'Bookings', desc: 'All bookings with host/renter/amount', icon: 'CalendarIcon', color: 'cyan' },        { id: 'financials', label: 'Financials', desc: 'Revenue log with fees + host earnings (1099-friendly)', icon: 'DollarSignIcon', color: 'emerald' },        { id: 'users', label: 'Users', desc: 'User list with LTV + counts', icon: 'UsersIcon', color: 'indigo' },        { id: 'listings', label: 'Listings', desc: 'All listings with moderation status', icon: 'PackageIcon', color: 'amber' },        { id: 'boosts', label: 'Boosts', desc: 'Boost revenue log', icon: 'RocketIcon', color: 'purple' },    ];    const download = (type: string) => {        setDownloading(type);        const url = `/api/admin/export-csv?type=${type}&admin_email=${encodeURIComponent(email)}&t=${Date.now()}`;        const a = document.createElement('a');        a.href = url;        a.download = `goodslister-${type}-${new Date().toISOString().slice(0,10)}.csv`;        document.body.appendChild(a);        a.click();        document.body.removeChild(a);        setTimeout(() => setDownloading(null), 2000);    };    return (        <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm mb-6">            <div className="flex items-center justify-between mb-4">                <div>                    <h3 className="text-lg font-black text-slate-900">Export CSV</h3>                    <p className="text-xs font-bold text-slate-500 mt-1">Download data for accounting, taxes, or analysis</p>                </div>            </div>            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">                {types.map(t => (                    <button key={t.id} onClick={() => download(t.id)} disabled={downloading === t.id} className={`p-4 rounded-xl border border-slate-100 hover:border-${t.color}-300 hover:bg-${t.color}-50 transition-all text-left disabled:opacity-50`}>                        <div className="flex items-center justify-between mb-1">                            <div className={`text-sm font-black text-${t.color}-700`}>{t.label}</div>                            {downloading === t.id ? <span className="text-xs font-bold text-slate-500">Downloading...</span> : <span className="text-xs font-black text-slate-400">CSV</span>}                        </div>                        <div className="text-xs font-bold text-slate-500">{t.desc}</div>                    </button>                ))}            </div>        </div>    );};const ModerationTab: React.FC = () => {    const [filter, setFilter] = React.useState<'pending' | 'rejected' | 'approved' | 'all'>('pending');    const [data, setData] = React.useState<any>(null);    const [loading, setLoading] = React.useState(true);    const [actioningId, setActioningId] = React.useState<string | null>(null);    const load = () => { setLoading(true); fetch(`/api/admin/listings-moderation?filter=${filter}&t=${Date.now()}`).then(r => r.json()).then(setData).catch(console.error).finally(() => setLoading(false)); };    React.useEffect(load, [filter]);    const act = async (listing_id: string, action: 'approve' | 'reject') => {        let reason = null;        if (action === 'reject') { reason = prompt('Rejection reason (visible to host):'); if (!reason) return; }        setActioningId(listing_id);        try { const r = await fetch('/api/admin/listings-moderation', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ admin_email: 'lucianoreverberi@gmail.com', listing_id, action, reason }) }); const j = await r.json(); if (j.success) load(); else alert('Error: ' + (j.error || 'unknown')); } catch (e: any) { alert('Error: ' + e.message); } finally { setActioningId(null); }    };    return (        <div className="space-y-6">            <div className="flex items-center justify-between">                <h2 className="text-2xl font-black text-slate-900">Listings Moderation</h2>                <button onClick={load} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-lg text-sm font-black text-slate-700 transition-all">Refresh</button>            </div>            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">                {(['pending', 'approved', 'rejected', 'all'] as const).map(f => (                    <button key={f} onClick={() => setFilter(f)} className={`p-4 rounded-2xl border-2 transition-all text-left ${filter === f ? 'bg-cyan-50 border-cyan-500 shadow-sm' : 'bg-white border-slate-100 hover:border-slate-200'}`}>                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-wider">{f}</div>                        <div className="text-3xl font-black text-slate-900 mt-1">{data?.counts?.[f] ?? (f === 'all' ? data?.counts?.total : '-')}</div>                    </button>                ))}            </div>            {loading && <div className="text-center py-8 text-slate-400 font-bold">Loading...</div>}            {!loading && data?.listings?.length === 0 && <div className="bg-white rounded-2xl p-12 text-center border border-slate-100"><div className="text-lg font-black text-slate-500">No listings to review</div><div className="text-sm font-bold text-slate-400 mt-2">You are all caught up in this filter.</div></div>}            {!loading && data?.listings?.length > 0 && (                <div className="space-y-3">                    {data.listings.map((l: any) => {                        const imgFirst = Array.isArray(l.images) ? l.images[0] : (typeof l.images === 'string' ? (l.images.startsWith('[') ? JSON.parse(l.images)[0] : l.images.split(',')[0]) : '');                        return (                            <div key={l.id} className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm hover:shadow-md transition-all flex gap-4 items-start">                                {imgFirst && <img src={imgFirst} alt={l.title} className="w-24 h-24 rounded-xl object-cover flex-shrink-0" />}                                <div className="flex-1 min-w-0">                                    <div className="flex items-start justify-between gap-3">                                        <div className="flex-1 min-w-0">                                            <h3 className="text-base font-black text-slate-900 truncate">{l.title}</h3>                                            <div className="text-xs font-bold text-slate-500 mt-1">{l.category} - {l.location_city}, {l.location_state}</div>                                            <div className="text-sm font-bold text-slate-700 mt-1">${l.price_per_day}/day{l.price_per_hour ? ` - $${l.price_per_hour}/hr` : ''}</div>                                        </div>                                        <span className={`text-[9px] font-black px-2 py-1 rounded-full uppercase whitespace-nowrap ${l.moderation_status === 'approved' ? 'bg-emerald-100 text-emerald-700' : l.moderation_status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{l.moderation_status}</span>                                    </div>                                    <p className="text-xs text-slate-600 mt-2 line-clamp-2">{l.description}</p>                                    {l.moderation_rejection_reason && <div className="mt-2 text-xs bg-red-50 border border-red-100 rounded-lg px-3 py-2 text-red-700"><span className="font-black">Reason:</span> {l.moderation_rejection_reason}</div>}                                    <div className="flex items-center gap-2 mt-3">                                        {l.moderation_status !== 'approved' && <button onClick={() => act(l.id, 'approve')} disabled={actioningId === l.id} className="px-4 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-black rounded-lg transition-all disabled:opacity-50">Approve</button>}                                        {l.moderation_status !== 'rejected' && <button onClick={() => act(l.id, 'reject')} disabled={actioningId === l.id} className="px-4 py-1.5 bg-red-100 hover:bg-red-200 text-red-700 text-xs font-black rounded-lg transition-all disabled:opacity-50">Reject</button>}                                        <span className="text-[10px] font-mono text-slate-400 ml-2">{l.id}</span>                                    </div>                                </div>                            </div>                        );                    })}                </div>            )}        </div>    );};const AdminPage: React.FC<AdminPageProps> = ({ 
    users, 
    listings, 
    bookings,
    heroSlides, 
    ...
    const tabs: { id: AdminTab; name: string; icon: React.ElementType }[] = [
        ...
        { id: 'moderation', name: 'Moderation', icon: ShieldIcon },        { id: 'users', name: 'Users', icon: UsersIcon },
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'dashboard': return <DashboardTab onNavigate={setActiveTab} />;            case 'moderation': return <ModerationTab />;
            case 'bookings':
                return <BookingsTab bookings={bookings} onUpdateDepositStatus={onUpdateDepositStatus} />;
            case 'financials':
                return (<div className="space-y-6"><ExportCsvSection /><FinancialsTab /></div>);
            case 'risk_fund':
                return <RiskFundTab />;
            case 'marketing':
                return <MarketingTab />;
