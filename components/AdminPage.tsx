                                    ...
                                    {filteredUsers.map(user => (
                                        <tr key={user.id} className="border-b last:border-0 hover:bg-slate-50/50">
                                            ...
                                            <td className="p-4 text-right">
                                                <div className="flex justify-end gap-2">
                                                    <button 
                                                        onClick={() => { const e=enrichedUsers[user.id]||{}; const f=userFlags[user.id]||{}; const w=window.open('','_blank','width=650,height=800,scrollbars=yes'); if(!w){alert('Popup blocked. Please allow popups for this site.');return;} w.document.write(`<html><head><title>Profile - ${user.name}</title><style>body{font-family:system-ui,-apple-system,sans-serif;padding:24px;background:#f8fafc;margin:0}.card{background:white;padding:28px;border-radius:20px;box-shadow:0 4px 32px rgba(0,0,0,.08);max-width:600px;margin:0 auto}h1{font-size:24px;margin:0 0 4px;color:#0f172a}.email{color:#64748b;margin-bottom:16px;font-size:14px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:24px 0}.stat{background:#f1f5f9;padding:16px;border-radius:12px;text-align:center}.stat-label{font-size:10px;color:#64748b;text-transform:uppercase;font-weight:700;letter-spacing:.05em}.stat-value{font-size:26px;font-weight:900;color:#0f172a;margin-top:4px}.id{font-family:monospace;font-size:11px;color:#94a3b8;word-break:break-all;background:#f8fafc;padding:10px;border-radius:8px;border:1px solid #e2e8f0}.badge{display:inline-block;padding:4px 10px;border-radius:12px;font-size:10px;font-weight:900;margin-right:6px;text-transform:uppercase;letter-spacing:.05em}.verified{background:#dcfce7;color:#166534}.unverified{background:#f1f5f9;color:#64748b}.superhost{background:#fef3c7;color:#92400e}.section{margin-top:20px}.section-label{font-size:10px;color:#64748b;text-transform:uppercase;font-weight:700;letter-spacing:.05em;margin-bottom:6px}.value{font-size:14px;color:#0f172a}</style></head><body><div class="card"><h1>${user.name||'Unknown'}</h1><div class="email">${user.email||''}</div><div>${user.isIdVerified?'<span class="badge verified">â Verified</span>':'<span class="badge unverified">Unverified</span>'}${f.is_superhost?'<span class="badge superhost">â­ Superhost</span>':''}</div><div class="grid"><div class="stat"><div class="stat-label">Bookings</div><div class="stat-value">${e.bookings_count||0}</div></div><div class="stat"><div class="stat-label">Boosts</div><div class="stat-value">${e.boosts_count||0}</div></div><div class="stat"><div class="stat-label">LTV</div><div class="stat-value">$${(e.ltv||0).toFixed(2)}</div></div><div class="stat"><div class="stat-label">Listings</div><div class="stat-value">${e.listings_count||0}</div></div></div><div class="section"><div class="section-label">Firebase UID</div><div class="id">${user.id}</div></div><div class="section"><div class="section-label">Last Activity</div><div class="value">${e.last_activity?new Date(e.last_activity).toLocaleString():'Never'}</div></div><div class="section"><div class="section-label">Role</div><div class="value">${user.role||'USER'}</div></div></div></body></html>`); w.document.close();}} className="px-3 py-1 bg-cyan-100 text-cyan-700 text-[10px] font-black rounded-lg hover:bg-cyan-200 transition-all uppercase" title="View Profile">👁️</button> <button className="px-3 py-1 bg-indigo-500 text-white text-[10px] font-black rounded-lg hover:bg-indigo-600 transition-all" 
