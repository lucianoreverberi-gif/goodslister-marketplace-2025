...
const ExplorePage: React.FC<ExplorePageProps> = ({ 
    ...
    const filteredAndSortedListings = useMemo(() => {
        let filtered = listings.filter(listing => {
            ...
            const minNum = minPrice ? parseFloat(minPrice) : 0; const maxNum = maxPrice ? parseFloat(maxPrice) : Infinity; const priceHourly = listing.pricePerHour || 0; const priceDaily = listing.pricePerDay || 0; let matchesPrice = true; if (pricingMode === 'daily') { matchesPrice = priceDaily > 0 && priceDaily >= minNum && priceDaily <= maxNum; } else if (pricingMode === 'hourly') { matchesPrice = priceHourly > 0 && priceHourly >= minNum && priceHourly <= maxNum; } else { const priceAny = priceDaily || priceHourly; matchesPrice = priceAny === 0 || (priceAny >= minNum && priceAny <= maxNum); } const matchesRating = minRating === 0 || (listing.rating || 0) >= minRating; let matchesDates = true; if (dateRange?.from && dateRange?.to) { const bookedDates = listing.bookedDates || []; const requestedInterval = { start: dateRange.from, end: dateRange.to }; const hasConflict = bookedDates.some((d) => { const bookedDay = typeof d === 'string' ? parseISO(d) : new Date(d); return isWithinInterval(bookedDay, requestedInterval); }); matchesDates = !hasConflict; }
