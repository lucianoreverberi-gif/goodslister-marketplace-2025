...
const UserDashboardPage: React.FC<UserDashboardPageProps> = (props) => {
    ...
    const renderContent = () => {
        switch (activeTab) {            case 'overview':                return (                    <div className="space-y-6 animate-in fade-in">                        <div className="bg-gradient-to-br from-cyan-500 to-blue-600 rounded-[2rem] p-8 text-white shadow-lg">                            <h2 className="text-3xl font-black">Welcome back, {user.name.split(' ')[0]}! ð</h2>                            <p className="text-cyan-100 text-sm mt-1 font-bold">{new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}</p>                        </div>                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">                            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all">                                <div className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Earnings This Month</div>                                <div className="text-3xl font-black text-emerald-600 mt-2">${(overviewData?.earnings?.this_month ?? 0).toFixed(2)}</div>                                {overviewData?.earnings?.change_pct !== undefined && overviewData.earnings.change_pct !== 0 && (                                    <div className={`text-xs font-black mt-2 ${overviewData.earnings.change_pct >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{overviewData.earnings.change_pct >= 0 ? 'â' : 'â'} {Math.abs(overviewData.earnings.change_pct)}% vs last month</div>                                )}                                {(!overviewData?.earnings?.change_pct || overviewData.earnings.change_pct === 0) && overviewData?.earnings?.this_month === 0 && (                                    <div className="text-xs font-bold mt-2 text-slate-400">Start renting to earn ð°</div>                                )}                            </div>                            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all">                                <div className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Bookings Received</div>                                <div className="text-3xl font-black text-slate-900 mt-2">{overviewData?.bookings_as_host?.total ?? 0}</div>                                {(overviewData?.bookings_as_host?.pending ?? 0) > 0 ? (                                    <div className="text-xs font-black mt-2 text-amber-600">ð {overviewData.bookings_as_host.pending} pending your response</div>                                ) : (                                    <div className="text-xs font-bold mt-2 text-slate-400">All caught up â</div>                                )}                            </div>                            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all">                                <div className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Active Listings</div>                                <div className="text-3xl font-black text-slate-900 mt-2">{overviewData?.listings?.active ?? 0}</div>                                {(overviewData?.listings?.active_boosts ?? 0) > 0 ? (                                    <div className="text-xs font-black mt-2 text-indigo-600">ð {overviewData.listings.active_boosts} boosted</div>                                ) : (overviewData?.listings?.active ?? 0) > 0 ? (                                    <div className="text-xs font-bold mt-2 text-slate-400">Try boosting to get more views</div>                                ) : (                                    <div className="text-xs font-bold mt-2 text-slate-400">Create your first listing â</div>                                )}                            </div>                            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all">                                <div className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Superhost Status</div>                                {overviewData?.superhost?.is_superhost ? (                                    <>                                        <div className="text-2xl font-black text-amber-600 mt-2">â­ Superhost</div>                                        <div className="text-xs font-bold mt-2 text-slate-500">Congrats! ð</div>                                    </>                                ) : (                                    <>                                        <div className="text-2xl font-black text-slate-900 mt-2">{overviewData?.superhost?.completed_bookings ?? 0}/{overviewData?.superhost?.goal ?? 10}</div>                                        <div className="w-full bg-slate-100 rounded-full h-2 mt-2 overflow-hidden"><div className="h-full bg-gradient-to-r from-amber-400 to-amber-600 transition-all" style={{width: `${overviewData?.superhost?.progress_pct ?? 0}%`}}></div></div>                                        <div className="text-xs font-bold mt-2 text-slate-400">{overviewData?.superhost?.bookings_needed ?? 10} more bookings to unlock</div>                                    </>                                )}                            </div>                        </div>                        <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">                            <h3 className="text-lg font-black text-slate-900 mb-4">Quick Actions</h3>                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">                                <button onClick={() => setActiveTab('listings')} className="p-4 bg-cyan-50 hover:bg-cyan-100 rounded-xl transition-all text-center border border-cyan-100"><div className="text-2xl mb-1">ð¦</div><div className="text-xs font-black text-slate-700">My Listings</div></button>                                <button onClick={() => setActiveTab('bookings')} className="p-4 bg-emerald-50 hover:bg-emerald-100 rounded-xl transition-all text-center border border-emerald-100"><div className="text-2xl mb-1">ð</div><div className="text-xs font-black text-slate-700">View Bookings</div></button>                                <button onClick={() => setActiveTab('boosts')} className="p-4 bg-indigo-50 hover:bg-indigo-100 rounded-xl transition-all text-center border border-indigo-100"><div className="text-2xl mb-1">ð</div><div className="text-xs font-black text-slate-700">Manage Boosts</div></button>                                <button onClick={() => window.open('/#userProfile/' + user.id, '_blank')} className="p-4 bg-slate-50 hover:bg-slate-100 rounded-xl transition-all text-center border border-slate-100"><div className="text-2xl mb-1">ð¤</div><div className="text-xs font-black text-slate-700">Public Profile</div></button>                            </div>                        </div>                    </div>                );            case 'profile':
                return (
