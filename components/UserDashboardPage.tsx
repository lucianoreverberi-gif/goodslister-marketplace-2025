...
import { PackageIcon, DollarSignIcon, BarChartIcon, BrainCircuitIcon, StarIcon, ShieldIcon, MailIcon, PhoneIcon, CreditCardIcon, CheckCircleIcon, CalendarIcon, EyeIcon, PencilIcon, XIcon, LandmarkIcon, CalculatorIcon, UmbrellaIcon, SmartphoneIcon, CameraFaceIcon, ScanIcon, FileWarningIcon, GavelIcon, CameraIcon, HeartIcon, UserCheckIcon, TrashIcon, AlertTriangleIcon, RocketIcon, ZapIcon, LockIcon, MapPinIcon, WandSparklesIcon, MegaphoneIcon, SparklesIcon, TrendUpIcon, ArrowRightIcon, RefreshCwIcon, LightbulbIcon, ClockIcon, SlidersIcon, ShieldCheckIcon, InfoIcon, ExternalLinkIcon, FileTextIcon, CompassIcon, ChevronRightIcon } from './icons';
...
const ProfileCompletenessWidget: React.FC<{ user: Session, compact?: boolean }> = ({ user, compact = false }) => {    const items = [        { label: 'Name', done: !!user.name && user.name.length > 0 },        { label: 'Bio', done: !!user.bio && user.bio.length >= 20 },        { label: 'Profile photo', done: !!user.avatarUrl && user.avatarUrl.length > 20 },        { label: 'Email verified', done: user.isEmailVerified === true },        { label: 'Phone verified', done: user.isPhoneVerified === true },        { label: 'Identity verified', done: user.isIdVerified === true },    ];    const doneCount = items.filter(i => i.done).length;    const total = items.length;    const pct = Math.round((doneCount / total) * 100);    const isComplete = doneCount === total;    if (compact) {        if (isComplete) return null;        return (            <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">                <div className="flex items-center justify-between mb-2">                    <h4 className="text-sm font-black text-slate-900">Complete your profile</h4>                    <span className="text-sm font-black text-cyan-600">{doneCount}/{total}</span>                </div>                <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden">                    <div className="h-full bg-gradient-to-r from-cyan-400 to-cyan-600 transition-all" style={{width: `${pct}%`}}></div>                </div>                <p className="mt-3 text-xs font-bold text-slate-500">Complete profiles get more bookings.</p>            </div>        );    }    return (        <div className="bg-gradient-to-br from-cyan-50 to-blue-50 rounded-2xl p-6 border border-cyan-100 mb-6">            <div className="flex items-center justify-between mb-4">                <div><h3 className="text-lg font-black text-slate-900">Profile Completeness</h3><p className="text-xs font-bold text-slate-500 mt-1">Complete profiles get 2x more bookings on average</p></div>                <div className={`text-3xl font-black ${isComplete ? 'text-emerald-600' : 'text-cyan-600'}`}>{pct}%</div>            </div>            <div className="w-full bg-white rounded-full h-3 overflow-hidden mb-4"><div className={`h-full transition-all ${isComplete ? 'bg-emerald-500' : 'bg-gradient-to-r from-cyan-400 to-cyan-600'}`} style={{width: `${pct}%`}}></div></div>            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">                {items.map(item => (<div key={item.label} className={`flex items-center gap-2 text-xs font-bold ${item.done ? 'text-emerald-700' : 'text-slate-500'}`}>{item.done ? <CheckCircleIcon className="h-4 w-4 text-emerald-500 flex-shrink-0" /> : <div className="h-4 w-4 rounded-full border-2 border-slate-300 flex-shrink-0" />} {item.label}</div>))}            </div>        </div>    );};const MyBoostsManager: React.FC<{ user: Session, onBoostListing: () => void }> = ({ user, onBoostListing }) => {
...
const BookingsManager: React.FC<{ 
    ...
    return (
        <div className="animate-in fade-in duration-500">
            ...
            {damageReportBooking && (                 <DamageReportModal                    bookingId={damageReportBooking.id}                    reporterId={userId}                    reporterRole={mode === 'renting' ? 'renter' : 'host'}                    depositAmount={damageReportBooking.securityDeposit || 0}                    onClose={() => setDamageReportBooking(null)}                    onSuccess={() => setDamageReportBooking(null)}                 />             )}             <div className="mb-6"><h2 className="text-2xl font-bold text-slate-800 mb-4">{mode === 'renting' ? 'My Trips' : 'Reservations'}</h2><div className="flex items-center gap-2 mb-3"><ChevronRightIcon className="h-3 w-3 text-cyan-600" /><p className="text-[10px] font-black text-cyan-600 uppercase tracking-widest">Switch view</p></div><div className="grid grid-cols-2 gap-3"><button onClick={() => setMode('renting')} className={`relative p-4 rounded-2xl border-2 transition-all text-left overflow-hidden ${mode === 'renting' ? 'bg-gradient-to-br from-cyan-50 to-white border-cyan-500 shadow-lg shadow-cyan-100 scale-[1.02]' : 'bg-white border-slate-200 hover:border-cyan-300 hover:bg-cyan-50/30 hover:scale-[1.01]'}`}>{mode === 'renting' && <div className="absolute top-2 right-2"><CheckCircleIcon className="h-4 w-4 text-cyan-600" /></div>}<div className="flex items-center gap-2 mb-2"><div className={`h-8 w-8 rounded-xl flex items-center justify-center transition-all ${mode === 'renting' ? 'bg-cyan-600 text-white shadow-md' : 'bg-slate-100 text-slate-400'}`}><CompassIcon className="h-4 w-4" /></div><span className={`text-[11px] font-black uppercase tracking-widest transition-colors ${mode === 'renting' ? 'text-cyan-700' : 'text-slate-500'}`}>Renting</span></div><div className="flex items-baseline justify-between"><div className={`text-xs font-bold transition-colors ${mode === 'renting' ? 'text-slate-700' : 'text-slate-500'}`}>Adventures I'm renting</div><span className={`text-3xl font-black transition-colors ${mode === 'renting' ? 'text-cyan-700' : 'text-slate-900'}`}>{rentingCount}</span></div></button><button onClick={() => setMode('hosting')} className={`relative p-4 rounded-2xl border-2 transition-all text-left overflow-hidden ${mode === 'hosting' ? 'bg-gradient-to-br from-indigo-50 to-white border-indigo-500 shadow-lg shadow-indigo-100 scale-[1.02]' : 'bg-white border-slate-200 hover:border-indigo-300 hover:bg-indigo-50/30 hover:scale-[1.01]'}`}>{mode === 'hosting' && <div className="absolute top-2 right-2"><CheckCircleIcon className="h-4 w-4 text-indigo-600" /></div>}<div className="flex items-center gap-2 mb-2"><div className={`h-8 w-8 rounded-xl flex items-center justify-center transition-all ${mode === 'hosting' ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-400'}`}><RocketIcon className="h-4 w-4" /></div><span className={`text-[11px] font-black uppercase tracking-widest transition-colors ${mode === 'hosting' ? 'text-indigo-700' : 'text-slate-500'}`}>Hosting</span>{pendingHostCount > 0 && <span className="text-[9px] font-black px-2 py-0.5 rounded-full bg-amber-500 text-white whitespace-nowrap animate-pulse">{pendingHostCount} PENDING</span>}</div><div className="flex items-baseline justify-between"><div className={`text-xs font-bold transition-colors ${mode === 'hosting' ? 'text-slate-700' : 'text-slate-500'}`}>People renting my stuff</div><span className={`text-3xl font-black transition-colors ${mode === 'hosting' ? 'text-indigo-700' : 'text-slate-900'}`}>{hostingCount}</span></div></button></div></div>
...
const UserDashboardPage: React.FC<UserDashboardPageProps> = (props) => {
    ...
    const renderContent = () => {
        switch (activeTab) {            case 'overview':                return (                    <div className="space-y-6 animate-in fade-in">                        <div className="bg-gradient-to-br from-cyan-500 to-blue-600 rounded-[2rem] p-8 text-white shadow-lg">                            <h2 className="text-3xl font-black">Welcome back, {user.name.split(' ')[0]}!</h2>                            <p className="text-cyan-100 text-sm mt-1 font-bold">{new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}</p>                        </div>                        <ErrorBoundary fallback={<div className="bg-white rounded-2xl p-4 border border-amber-200 text-xs text-amber-700">Attention panel temporarily unavailable — check the Bookings tab for pending actions.</div>}>                            <AttentionPanel userId={user.id} bookings={bookings} justSignedIds={justSignedIds} onOpenSignContract={(b) => setSigningBooking(b)} onOpenCheckIn={(b) => { setActiveSessionBooking(b); setSessionInitialMode('handover'); }} onOpenReturn={(b) => { setActiveSessionBooking(b); setSessionInitialMode('return'); }} onOpenBookings={() => setActiveTab('bookings')} />                        </ErrorBoundary>                        <ProfileCompletenessWidget user={user} compact /><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">                            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all">                                <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-wider"><DollarSignIcon className="h-3 w-3" />Earnings This Month</div>                                <div className="text-3xl font-black text-emerald-600 mt-2">${(overviewData?.earnings?.this_month ?? 0).toFixed(2)}</div>                                {overviewData?.earnings?.change_pct !== undefined && overviewData.earnings.change_pct !== 0 && (                                    <div className={`text-xs font-black mt-2 ${overviewData.earnings.change_pct >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{overviewData.earnings.change_pct >= 0 ? '\u2191' : '\u2193'} {Math.abs(overviewData.earnings.change_pct)}% vs last month</div>                                )}                                {(!overviewData?.earnings?.change_pct || overviewData.earnings.change_pct === 0) && overviewData?.earnings?.this_month === 0 && (                                    <div className="text-xs font-bold mt-2 text-slate-400">Start renting to earn</div>                                )}                            </div>                            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all">                                <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-wider"><CalendarIcon className="h-3 w-3" />Bookings Received</div>                                <div className="text-3xl font-black text-slate-900 mt-2">{overviewData?.bookings_as_host?.total ?? 0}</div>                                {(overviewData?.bookings_as_host?.pending ?? 0) > 0 ? (                                    <div className="text-xs font-black mt-2 text-amber-600">{overviewData.bookings_as_host.pending} pending your response</div>                                ) : (overviewData?.bookings_as_renter?.completed ?? 0) > 0 ? (                                    <div className="text-xs font-black mt-2 text-cyan-600">✈ {overviewData.bookings_as_renter.completed} trip{overviewData.bookings_as_renter.completed > 1 ? 's' : ''} completed as guest</div>                                ) : (                                    <div className="text-xs font-bold mt-2 text-slate-400">All caught up</div>                                )}                            </div>                            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all">                                <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-wider"><PackageIcon className="h-3 w-3" />Active Listings</div>                                <div className="text-3xl font-black text-slate-900 mt-2">{overviewData?.listings?.active ?? 0}</div>                                {(overviewData?.listings?.active_boosts ?? 0) > 0 ? (                                    <div className="text-xs font-black mt-2 text-indigo-600">{overviewData.listings.active_boosts} boosted</div>                                ) : (overviewData?.listings?.active ?? 0) > 0 ? (                                    <div className="text-xs font-bold mt-2 text-slate-400">Try boosting to get more views</div>                                ) : (                                    <div className="text-xs font-bold mt-2 text-slate-400">Create your first listing</div>                                )}                            </div>                            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all">                                <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-wider"><StarIcon className="h-3 w-3" />Superhost Status</div>                                {overviewData?.superhost?.is_superhost ? (                                    <>                                        <div className="text-2xl font-black text-amber-600 mt-2">Superhost</div>                                        <div className="text-xs font-bold mt-2 text-slate-500">Congrats!</div>                                    </>                                ) : (                                    <>                                        <div className="text-2xl font-black text-slate-900 mt-2">{overviewData?.superhost?.completed_bookings ?? 0}/{overviewData?.superhost?.goal ?? 10}</div>                                        <div className="w-full bg-slate-100 rounded-full h-2 mt-2 overflow-hidden"><div className="h-full bg-gradient-to-r from-amber-400 to-amber-600 transition-all" style={{width: `${overviewData?.superhost?.progress_pct ?? 0}%`}}></div></div>                                        <div className="text-xs font-bold mt-2 text-slate-400">{overviewData?.superhost?.bookings_needed ?? 10} more bookings to unlock</div>                                    </>                                )}                            </div>                        </div>                        <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">                            <h3 className="text-lg font-black text-slate-900 mb-4">Quick Actions</h3>                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">                                <button onClick={() => setActiveTab('listings')} className="p-4 bg-cyan-50 hover:bg-cyan-100 rounded-xl transition-all text-center border border-cyan-100 flex flex-col items-center gap-2"><PackageIcon className="h-6 w-6 text-cyan-600" /><div className="text-xs font-black text-slate-700">My Listings</div></button>                                <button onClick={() => setActiveTab('bookings')} className="p-4 bg-emerald-50 hover:bg-emerald-100 rounded-xl transition-all text-center border border-emerald-100 flex flex-col items-center gap-2"><CalendarIcon className="h-6 w-6 text-emerald-600" /><div className="text-xs font-black text-slate-700">View Bookings</div></button>                                <button onClick={() => setActiveTab('boosts')} className="p-4 bg-indigo-50 hover:bg-indigo-100 rounded-xl transition-all text-center border border-indigo-100 flex flex-col items-center gap-2"><RocketIcon className="h-6 w-6 text-indigo-600" /><div className="text-xs font-black text-slate-700">Manage Boosts</div></button>                                <button onClick={() => window.open('/#userProfile/' + user.id, '_blank')} className="p-4 bg-slate-50 hover:bg-slate-100 rounded-xl transition-all text-center border border-slate-100 flex flex-col items-center gap-2"><UserCheckIcon className="h-6 w-6 text-slate-600" /><div className="text-xs font-black text-slate-700">Public Profile</div></button>                            </div>                        </div>                    </div>                );            case 'profile':
                return (
                    <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 p-8 space-y-8 animate-in fade-in"><ProfileCompletenessWidget user={user} />
