...
export default async function handler(
  ...
  try {
    ...
    switch (type) {
      ...
      case 'damage_report_submitted': { subject = 'A damage report was submitted against you'; html = getBaseTemplate(`<h1>Damage Report Submitted</h1><p>Hi ${data.userName || 'there'},</p><p><strong>${data.reporterName}</strong> has submitted a damage report on your recent rental for <strong>$${data.claimedAmount}</strong>.</p><p><em>Description:</em> ${data.description}</p><p>You have <strong>48 hours</strong> to respond by accepting or disputing. If you don't respond, the claim will be automatically accepted.</p>`, 'Review Report', `https://goodslister.com/#damage/${data.reportId}`); break; }      case 'damage_accepted': { subject = 'Your damage report was accepted'; html = getBaseTemplate(`<h1>Report Accepted</h1><p>Hi ${data.userName || 'there'},</p><p><strong>${data.reportedName}</strong> has accepted your damage report of <strong>$${data.claimedAmount}</strong>. Our team will now review and process the payment.</p>`, 'View Report', `https://goodslister.com/#damage/${data.reportId}`); break; }      case 'damage_disputed': { subject = 'A damage report is being disputed'; html = getBaseTemplate(`<h1>Damage Report Disputed</h1><p>A damage report needs admin review.</p><p><strong>Reporter:</strong> ${data.reporterName}<br><strong>Reported:</strong> ${data.reportedName}<br><strong>Claimed:</strong> $${data.claimedAmount}</p><p><em>Dispute:</em> ${data.disputeNote}</p>`, 'Open Admin Center', `https://goodslister.com/#admin/damage/${data.reportId}`); break; }      case 'damage_captured': { subject = 'Damage claim resolved - deposit captured'; html = getBaseTemplate(`<h1>Deposit Captured</h1><p>Hi ${data.userName || 'there'},</p><p>Our team has reviewed the damage report and captured <strong>$${data.amount}</strong> from your security deposit (claimed: $${data.claimedAmount}).</p><p><em>Admin notes:</em> ${data.adminNotes || 'None'}</p>`, 'View Details', `https://goodslister.com/#damage/${data.reportId}`); break; }      case 'damage_released': { subject = 'Damage claim resolved - deposit released'; html = getBaseTemplate(`<h1>Deposit Released</h1><p>Hi ${data.userName || 'there'},</p><p>Great news! Our team reviewed the damage report and decided to release your full security deposit. No charges will be applied.</p>`, 'Explore Listings', 'https://goodslister.com/#explore'); break; }      default:
