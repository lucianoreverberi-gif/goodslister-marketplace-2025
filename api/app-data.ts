  ...
  try {
    ...
    const listings = listingsQuery.rows.map(row => {
      ...
      return {
        ...
        securityDeposit: Number(row.deposit_amount),
        security_deposit: Number(row.deposit_amount),
        operation_mode: row.operation_mode ? (row.operation_mode as any) : undefined,
        item_value: row.item_value ? Number(row.item_value) : undefined,
